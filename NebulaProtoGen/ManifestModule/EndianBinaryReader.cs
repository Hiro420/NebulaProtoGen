...
public class EndianBinaryReader : BinaryReader
{
	...
	public List<bool> ReadExcelBitfield(bool isSmall = false)
	{
		var res = new List<bool>();

		if (isSmall)
		{
			byte curByte = base.ReadByte();
			for (int i = 0; i < 7; i++)
			{
				res.Add((curByte & 0x1) != 0);
				curByte >>= 1;
			}
			return res;
		}
	...
	public string ReadStringSignedVarInt()
	{
		long len = ReadSignedVarInt();
		byte[] bytes = new byte[len];
