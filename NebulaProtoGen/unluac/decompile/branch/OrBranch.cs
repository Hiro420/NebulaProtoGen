...
namespace unluac.decompile.branch
{
	public class OrBranch : Branch
	{
		...
		public override Branch Invert() { return new AndBranch(left.Invert(), right.Invert()); }
		public override int GetRegister() { int rleft = left.GetRegister(); int rright = right.GetRegister(); return rleft == rright ? rleft : -1; }
		public override Expression AsExpression(Registers r) { return new expression.BinaryExpression("or", left.AsExpression(r), right.AsExpression(r), Expression.PRECEDENCE_OR, Expression.ASSOCIATIVITY_NONE); }
